...
/////////////////////////////////////////////////串口通讯定义
bit    busy;
bit manualMode = 0;    // 0:自动循迹模式  1:蓝牙手动模式
uchar manualCmd = 0;   // 手动指令：0停止 1前进 2后退 3左转 4右转
uchar  wptr;
uchar  rptr;
...
void main()
{
    ...
    while(1)
    {
        ...
        if(BT != 0)
        {
            switch(BT)
            {
                case 0x07: manualMode = 1; manualCmd = 1; break; // 前进（手动）
                case 0x08: manualMode = 1; manualCmd = 2; break; // 后退（手动）
                case 0x09: manualMode = 1; manualCmd = 3; break; // 左转（手动）
                case 0x0A: manualMode = 1; manualCmd = 4; break; // 右转（手动）
                case 0x06: manualMode = 1; manualCmd = 0; break; // 停止（保持手动模式）
                case 0x05: manualMode = 0; manualCmd = 0; stop(); break; // 恢复自动循迹
                default: break;
            }
            BT = 0;  // 处理完毕，清零
        }
       ...
       if(manualMode)
{
    ...
    switch(manualCmd)
    {
        ...
        case 2: houtui();    break;   // 后退
        case 3: leftArc();   break;   // 带前进的左转 ← 改为新函数
        case 4: rightArc();  break;   // 带前进的右转 ← 改为新函数
...
{
    // 自动循迹模式，转弯仍用原 zuozhuan() / youzhuan()，不动
    CheckTrafficLight();
    CheckRFID();
    FollowLine();
